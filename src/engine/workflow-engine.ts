...
export class WorkflowEngine {
  ...
  private async runStage(
    ...
    const systemPrompt = `You are the ${stage} agent for Laravel Harness V2.`;
  ...
  private writeArtifact(stage: Stage, state: RunState, result: StageResult): void {
    ...
    fs.writeFileSync(artifactPath, result.summary, 'utf8');

    // Also write to run root for easy access
    fs.writeFileSync(path.join(this.opts.paths.runDir, artifactName), result.summary, 'utf8');
